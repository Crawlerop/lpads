...
def main():
    ...
    NOTFOUNDAD = []
    if len(sys.argv) > 2:
        if sys.argv[2] == "-keepsearch":
            failnoad = False
        elif sys.argv[2] == "-proxy":
            PROXY_SITE = sys.argv[3]
        elif sys.argv[2] == "-outp":
            OUTP = sys.argv[3]
    if ((PROXY_SITE or OUTP) and len(sys.argv) > 4) or (len(sys.argv) > 3):
        if (PROXY_SITE or OUTP):
            if sys.argv[4] == "-keepsearch":
                failnoad = False
            ...
            if sys.argv[3] == "-keepsearch":
                failnoad = False
            elif sys.argv[3] == "-proxy":
                PROXY_SITE = sys.argv[4]
            elif sys.argv[3] == "-outp":
                OUTP = sys.argv[4]
    if ((PROXY_SITE or OUTP) and len(sys.argv) > 5) or (len(sys.argv) > 4):
        if (PROXY_SITE or OUTP):
            if sys.argv[5] == "-keepsearch":
                failnoad = False
    ...
    mdata = list(json.load(open(sys.argv[1], "r", encoding="utf-8")).values())
    
    # print(OUTP)

    if not OUTP:
        if PROXY_SITE:
            outfo = f"linead_px_{base64.encodebytes(PROXY_SITE.encode('utf-8')).decode('utf-8')[:-1]}_{datetime.datetime.now().strftime('%y-%m-%d_%H-%M-%S')}"
        else:
            outfo = f"linead_{datetime.datetime.now().strftime('%y-%m-%d_%H-%M-%S')}"
    else:
        outfo = OUTP
    ...
    si = 1
    indx = open(f"{outfo}/index.html", "w", encoding="utf-8")

    for mdat in mdata:       
        retries = 0
        skip = False
        lat, lng = tuple(urlparse(mdat['url']).path.split("/")[2][1:].split(",")[:2])
        mapad = locadslib.MapAds([None])
        startloc = f"{lat}, {lng}"
        tick = time.perf_counter()+0.05
        lt = 0
        print(f"Index {si}: Searching ads for {mdat['line-1']}, {mdat['line-2']} in {lat}, {lng}... [{loadani_chars[lt]}] 0 tries", end="\r")
        while mapad.noads:
            if time.perf_counter() >= tick:
                lt += 1
                if lt > 3:
                    lt = 0
                print(f"Index {si}: Searching ads for {mdat['line-1']}, {mdat['line-2']} in {lat}, {lng}... [{loadani_chars[lt]}] {retries} tries", end="\r")
            if PROXY_SITE:
            ...
            if mapad.noads and failnoad:
                if retries >= TRIES:
                    ...
                    NOTFOUNDAD.append(f"{startloc} - {mdat['line-1']} - {mdat['line-2']}")
        ...
        outfn = f"{outfo}/{base64.encodebytes(startloc.encode('utf-8')).decode('utf-8')[:-1]}.html"

        open(outfn, "w", encoding="utf-8").write(outtemp)
        indx.write(f'''<a href="{base64.encodebytes(startloc.encode('utf-8')).decode('utf-8')[:-1]}.html">{startloc} - {mdat['line-1']} - {mdat['line-2']}</a><br>\n''')
        indx.flush()
    ...
    print("Location with no ads:")
    print()
    for na in NOTFOUNDAD:
        print(na)
    print()
