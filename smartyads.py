...
def main():
    ...
    qusts = json.load(open(sys.argv[1], "r", encoding="utf-8"))["items"]
    
    # print(OUTP)

    if not OUTP:
        if PROXY_SITE:
            outfo = f"smartad_px_{base64.encodebytes(PROXY_SITE.encode('utf-8')).decode('utf-8')[:-1]}_{datetime.datetime.now().strftime('%y-%m-%d_%H-%M-%S')}"
        else:
            outfo = f"smartad_{datetime.datetime.now().strftime('%y-%m-%d_%H-%M-%S')}"
    else:
        outfo = OUTP
    ...
    for qust in qusts:       
        retries = 0
        skip = False
        lat = qust["answer"]["location"]["lat"]
        lng = qust["answer"]["location"]["lng"]
        mapad = locadslib.MapAds([None])
        startloc = f"{lat}, {lng}"
        tick = time.perf_counter()+0.05
        lt = 0
        print(f"Index {si}: Searching ads for {qust['answer']['title']} - {qust['title']} in {lat}, {lng}... [{loadani_chars[lt]}] 0 tries", end="\r")
        while mapad.noads:
            ...
            if mapad.noads and failnoad:
                if retries >= TRIES:
                    ...
                    NOTFOUNDAD.append(f"{startloc} - {qust['answer']['title']} - {qust['title']}")
        ...
        indx.write(f'''<a href="{base64.encodebytes(startloc.encode('utf-8')).decode('utf-8')[:-1]}.html">{startloc} - {qust['answer']['title']} - {qust['title']}</a><br>\n''')
        indx.flush()
